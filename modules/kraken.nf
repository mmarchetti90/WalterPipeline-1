process Kraken {
  ...
  kraken2 --db ${params.kraken_database} --paired --gzip-compressed --threads 8 --report ${report} --use-names ${read1} ${read2} --output ${sample_id}.out
  ...
  grep -E 'Mycobacterium \(taxid 1763\)|Mycobacterium tuberculosis' ${sample_id}.out | awk '{print ${read2}}' > ${sample_id}_reads.list

  # use bbmap to select reads corresponding to taxa of interest.
  filterbyname.sh int=false in1=${read1} in2=${read2} out1=${sample_id}_kr_1.fq.gz out2=${sample_id}_kr_2.fq.gz names=${prefix}_reads.list include=true overwrite=true
