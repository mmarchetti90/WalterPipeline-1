process VariantsLoFreq {
  ...
  awk -F'\t' -v genotype=${GENOTYPE} -v OFS="\t" '/^[^#]/{ $9 = "DP"; $10 = genotype }1' | bgzip > ${sample_id}_lofreq_unfilt.vcf.gz
  ...
  awk -F'\t' -v genotype=${GENOTYPE} -v OFS="\t" '/^[^#]/{ $9 = "DP"; $10 = genotype }1' | bgzip > ${sample_id}_lofreq_filt.vcf.gz
